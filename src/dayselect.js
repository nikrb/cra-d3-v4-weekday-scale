...
export default function(mapFunction, weekday_scale) {
  ...
  function dayselect_scale(linear, methods, format, mapFunction) {
    ...
    scale.ticks = function(interval, skip) {
      var extent = dayselect_scaleExtent( weekday_scale.domain());
      ...
      //return
      var out = interval.range(mapFunction.invert(extent[0]), mapFunction.invert(+extent[1] + 1), skip < 1 ? 1 : skip); // inclusive upper bound
  ...
  var dayselect_time_scaleLocalMethods = [
    ...
    [d3.timeDay, 2],
    [d3.timeDay, 5], //.week, 1
    [d3.timeDay, 22], //.month, 1
    [d3.timeDay, 66], //.month, 3
    [d3.timeDay, 261] //.year, 1
  ];
  ...
  function dayselect_time_formatMulti(formats) {
    ...
    while (++i < n) {
      formats[i][0] = d3.timeFormat(formats[i][0]);
    }
  ...
  dayselect_time_scaleLocalMethods.year = d3.timeYear;

  return dayselect_scale(d3.scaleLinear(), dayselect_time_scaleLocalMethods, dayselect_time_scaleLocalFormat, mapFunction);
};
